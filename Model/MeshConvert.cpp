...
void OptimizeMesh( Renderer::Primitive& outPrim, const glTF::Primitive& inPrim, const Math::Matrix4& localToObject )
{
    ...
    if (inPrim.indices == nullptr)
    {
        ...
        // if index buffer not provided, then create one
    ...
    {
        // copy / convert index buffer data
        ...
        // determine the maximum index buffer value for size of index buffer values (16 or 32 bit)
        if (maxIndex == 0)
        {
    ...
    // fill out DirectX vertex attribute format structure
    ...
    // read / copy the vertex attributes data from inPrim to DirectX vertex buffers using DirectX::VBReader
    VBReader vbr;
    ...
    // Verify we have POSITION data, then compute bounding box and bounding sphere from input mesh bounding box
