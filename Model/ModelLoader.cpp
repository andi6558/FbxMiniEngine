...
std::shared_ptr<Model> Renderer::LoadModel(const std::wstring& filePath, bool forceRebuild)
{
    ...
    if (needBuild)
    {
        ...
        {
            Utility::Printf(L"---------------------------------------------------------------\n");
            Utility::Printf(L"Congratulations! You have found a fbx file!\n");
            Utility::Printf(L"The file path is : %ws\n", filePath.c_str());
            Utility::Printf(L"---------------------------------------------------------------\n");
            FBX::Asset asset(filePath);
            FBX::BuildModel(modelData, asset);
            //asset.PrintFBXFile(filePath);
            return nullptr;
        }
        else
        {
